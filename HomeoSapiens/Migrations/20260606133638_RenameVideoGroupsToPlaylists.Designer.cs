// <auto-generated />
using System;
using HomeoSapiens.Data;
using HomeoSapiens.Models.Enums;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace HomeoSapiens.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20260606133638_RenameVideoGroupsToPlaylists")]
    partial class RenameVideoGroupsToPlaylists
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.8")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "user_role", new[] { "admin", "regular" });
            NpgsqlModelBuilderExtensions.HasPostgresExtension(modelBuilder, "citext");
            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("HomeoSapiens.Models.Entities.Asset", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("ObjectKey")
                        .HasColumnType("text")
                        .HasColumnName("object_key");

                    b.Property<string>("OriginalFilename")
                        .HasColumnType("text")
                        .HasColumnName("original_filename");

                    b.Property<bool>("Scaled")
                        .HasColumnType("boolean")
                        .HasColumnName("scaled");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("pk_assets");

                    b.ToTable("assets", (string)null);
                });

            modelBuilder.Entity("HomeoSapiens.Models.Entities.Event", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("DescriptionEn")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description_en");

                    b.Property<string>("DescriptionPl")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description_pl");

                    b.Property<DateTime>("EndsAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ends_at");

                    b.Property<string>("Slug")
                        .IsRequired()
                        .HasColumnType("citext")
                        .HasColumnName("slug");

                    b.Property<DateTime>("StartsAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("starts_at");

                    b.Property<string>("TitleEn")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("title_en");

                    b.Property<string>("TitlePl")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("title_pl");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("pk_events");

                    b.HasIndex("Slug")
                        .IsUnique()
                        .HasDatabaseName("ix_events_slug");

                    b.ToTable("events", null, t =>
                        {
                            t.HasCheckConstraint("end_time_must_be_after_start_time", "(ends_at > starts_at)");
                        });
                });

            modelBuilder.Entity("HomeoSapiens.Models.Entities.Playlist", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("Slug")
                        .IsRequired()
                        .HasColumnType("citext")
                        .HasColumnName("slug");

                    b.Property<string>("TitleEn")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("title_en");

                    b.Property<string>("TitlePl")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("title_pl");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("pk_playlists");

                    b.HasIndex("Slug")
                        .IsUnique()
                        .HasDatabaseName("ix_playlists_slug");

                    b.ToTable("playlists", (string)null);
                });

            modelBuilder.Entity("HomeoSapiens.Models.Entities.PlaylistVideo", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid>("PlaylistId")
                        .HasColumnType("uuid")
                        .HasColumnName("playlist_id");

                    b.Property<int>("Position")
                        .HasColumnType("integer")
                        .HasColumnName("position");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<Guid>("VideoId")
                        .HasColumnType("uuid")
                        .HasColumnName("video_id");

                    b.HasKey("Id")
                        .HasName("pk_playlist_videos");

                    b.HasIndex("VideoId")
                        .HasDatabaseName("ix_playlist_videos_video_id");

                    b.HasIndex("PlaylistId", "Position")
                        .IsUnique()
                        .HasDatabaseName("ix_playlist_videos_playlist_id_position");

                    b.HasIndex("PlaylistId", "VideoId")
                        .IsUnique()
                        .HasDatabaseName("ix_playlist_videos_playlist_id_video_id");

                    b.ToTable("playlist_videos", null, t =>
                        {
                            t.HasCheckConstraint("playlist_video_position_must_be_non_neg", "position >= 0");
                        });
                });

            modelBuilder.Entity("HomeoSapiens.Models.Entities.User", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("citext")
                        .HasColumnName("email");

                    b.Property<string>("FamilyName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("family_name");

                    b.Property<string>("GivenName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("given_name");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("text")
                        .HasColumnName("password_hash");

                    b.Property<UserRole>("Role")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("user_role")
                        .HasDefaultValue(UserRole.Regular)
                        .HasColumnName("role");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("pk_users");

                    b.HasIndex("Email")
                        .IsUnique()
                        .HasDatabaseName("ix_users_email");

                    b.ToTable("users", (string)null);
                });

            modelBuilder.Entity("HomeoSapiens.Models.Entities.Video", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<int?>("DurationSeconds")
                        .HasColumnType("integer")
                        .HasColumnName("duration_seconds");

                    b.Property<string>("Slug")
                        .IsRequired()
                        .HasColumnType("citext")
                        .HasColumnName("slug");

                    b.Property<Guid?>("ThumbnailEnId")
                        .HasColumnType("uuid")
                        .HasColumnName("thumbnail_en_id");

                    b.Property<Guid?>("ThumbnailPlId")
                        .HasColumnType("uuid")
                        .HasColumnName("thumbnail_pl_id");

                    b.Property<string>("TitleEn")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("title_en");

                    b.Property<string>("TitlePl")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("title_pl");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("pk_videos");

                    b.HasIndex("ThumbnailEnId")
                        .HasDatabaseName("ix_videos_thumbnail_en_id");

                    b.HasIndex("ThumbnailPlId")
                        .HasDatabaseName("ix_videos_thumbnail_pl_id");

                    b.ToTable("videos", (string)null);
                });

            modelBuilder.Entity("HomeoSapiens.Models.Entities.VideoSource", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Codec")
                        .HasColumnType("text")
                        .HasColumnName("codec");

                    b.Property<string>("ContentType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("content_type");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("ObjectKey")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("object_key");

                    b.Property<int>("Position")
                        .HasColumnType("integer")
                        .HasColumnName("position");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<Guid>("VideoId")
                        .HasColumnType("uuid")
                        .HasColumnName("video_id");

                    b.HasKey("Id")
                        .HasName("pk_video_sources");

                    b.HasIndex("VideoId", "Position")
                        .IsUnique()
                        .HasDatabaseName("ix_video_sources_video_id_position");

                    b.ToTable("video_sources", null, t =>
                        {
                            t.HasCheckConstraint("video_source_position_must_be_non_neg", "position >= 0");
                        });
                });

            modelBuilder.Entity("HomeoSapiens.Models.Entities.PlaylistVideo", b =>
                {
                    b.HasOne("HomeoSapiens.Models.Entities.Playlist", "Playlist")
                        .WithMany("PlaylistVideos")
                        .HasForeignKey("PlaylistId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_playlist_videos_playlists_playlist_id");

                    b.HasOne("HomeoSapiens.Models.Entities.Video", "Video")
                        .WithMany("PlaylistVideos")
                        .HasForeignKey("VideoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_playlist_videos_videos_video_id");

                    b.Navigation("Playlist");

                    b.Navigation("Video");
                });

            modelBuilder.Entity("HomeoSapiens.Models.Entities.Video", b =>
                {
                    b.HasOne("HomeoSapiens.Models.Entities.Asset", "ThumbnailEn")
                        .WithMany()
                        .HasForeignKey("ThumbnailEnId")
                        .HasConstraintName("fk_videos_assets_thumbnail_en_id");

                    b.HasOne("HomeoSapiens.Models.Entities.Asset", "ThumbnailPl")
                        .WithMany()
                        .HasForeignKey("ThumbnailPlId")
                        .HasConstraintName("fk_videos_assets_thumbnail_pl_id");

                    b.Navigation("ThumbnailEn");

                    b.Navigation("ThumbnailPl");
                });

            modelBuilder.Entity("HomeoSapiens.Models.Entities.VideoSource", b =>
                {
                    b.HasOne("HomeoSapiens.Models.Entities.Video", "Video")
                        .WithMany()
                        .HasForeignKey("VideoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_video_sources_videos_video_id");

                    b.Navigation("Video");
                });

            modelBuilder.Entity("HomeoSapiens.Models.Entities.Playlist", b =>
                {
                    b.Navigation("PlaylistVideos");
                });

            modelBuilder.Entity("HomeoSapiens.Models.Entities.Video", b =>
                {
                    b.Navigation("PlaylistVideos");
                });
#pragma warning restore 612, 618
        }
    }
}
